...
export class ReportsController {
  ...
  async generateSummaryReport(@Res() res: any, @Query('limit') limit?: string) {
    try {
      ...
      const filePath = await this.reportsService.generateSummaryReport(limitNum);
  ...
  @Get('summary/xlsx')
  async generateSummaryReportXLSX(@Res() res: any, @Query('limit') limit?: string) {
    try {
      this.logger.log('Generating XLSX summary report...');
      
      const limitNum = limit ? parseInt(limit, 10) : undefined;
      const filePath = await this.reportsService.generateSummaryReportXLSX(limitNum);
      const filename = path.basename(filePath);
      ...
      this.logger.log(`XLSX summary report sent: ${filename}`);
    } catch (error) {
      this.logger.error(`Error generating XLSX summary report: ${error.message}`);
      res.status(500).json({ 
        error: 'Failed to generate XLSX summary report',
        message: error.message 
      });
    }
  }
